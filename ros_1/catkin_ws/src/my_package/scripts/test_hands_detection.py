...
class HandsDetectionNode:
    def __init__(self):
        ...
        self.no_dirt_wait_time = 4
        self.no_dirt_old_time = time.time()
    ...
    def detect_hand(self, image_message):
    ...
    def image_callback(self, image_message):
        ...
        if self.cleaning_state == 'no dirt':
            time_passed = time.time() - self.no_dirt_old_time
            if time_passed < self.no_dirt_wait_time:
                print(f'No dirt on the table. {int(time_passed)} seconds are passed. Waiting until {self.no_dirt_wait_time} seconds, then check if there are hands')
            else:
                self.no_dirt_old_time = time.time()
                print(f'{self.no_dirt_wait_time} seconds are passed. Detecting hands...')
                ret = self.detect_hand(image_message)
                if ret:
