...
class Refiner(nn.Module):
    ...
    Args:
        mode: area selection mode. Options:
            "full"         - No area selection, refine everywhere using regular Conv2d, but it's not recommended (too many noises).
    ...
    def forward(self,
        ...
        H_half, W_half = torch.div(H_full, 2, rounding_mode='floor'), torch.div(W_full, 2, rounding_mode='floor')
        H_quat, W_quat = torch.div(H_full, 4, rounding_mode='floor'), torch.div(W_full, 4, rounding_mode='floor')
    ...
    def select_refinement_regions(self, err: torch.Tensor):
        ...
        if self.mode == 'sampling':
            ...
            # The following k and idx definitions prevent errors in topk function, but
            # WARNING! It may throw errors in Tensorflow JS.
            k = min(torch.div(self.sample_pixels, 16, rounding_mode='floor'),
                          err.size()[1]).item()
            idx = err.topk(k, dim=1, sorted=False).indices
            # idx = err.topk(torch.div(self.sample_pixels, 16, rounding_mode='floor'), dim=1, sorted=False).indices
    ...
    def replace_patch(self,
        ...
        if self.patch_replace_method == 'scatter_nd':
            ...
            x = x.view(xB, xC, torch.div(xH, yH, rounding_mode='floor'),
                       yH, torch.div(xW, yW, rounding_mode='floor'), yW).permute(0, 2, 4, 1, 3, 5)
    ...
    def compute_pixel_indices(self,
        ...
        b, y, x = idx
        n = b.size(0)
        c = torch.arange(C).to(x.device)  # Fixs runtime PyTorch error for gpu
        o = torch.arange(O).to(x.device)  # Fixs runtime PyTorch error for gpu
